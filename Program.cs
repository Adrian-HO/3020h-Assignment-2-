...
namespace Treap
{
    ...
    public class Node<T> where T : IComparable
    {
        ...
        // additional data structure to store the minimum gap between adjacent elements
        private readonly Dictionary<Node<T>, int> minGap = new Dictionary<Node<T>, int>();
    ...
    class Treap<T> : ISearchable<T> where T : IComparable
    {
        ...
        private Node<T> Add(T item, Node<T> root)
        {
            ...
            else
            {
                ...
                if (cmp > 0)
                {
                    ...
                    // update min gap between root and its right child
                    if (!minGap.ContainsKey(root))
                    {
                        minGap[root] = int.MaxValue;
                    }
                    minGap[root] = Math.Min(minGap[root], Math.Abs((dynamic)root.Item - (dynamic)root.Right.Item));
                }
                else if (cmp < 0)
                {
                    ...
                    // update min gap between root and its left child
                    if (!minGap.ContainsKey(root))
                    {
                        minGap[root] = int.MaxValue;
                    }
                    minGap[root] = Math.Min(minGap[root], Math.Abs((dynamic)root.Item - (dynamic)root.Left.Item));
                }
        ...
        private int MinGap(Node<T> root)
        {
            if (root == null)
            {
                return int.MaxValue;
            ...
            if (root.Left != null)
            {
                ...
                leftMin = Math.Abs((dynamic)root.Item - (dynamic)current.Item);
            }
            ...
            if (root.Right != null)
            {
                ...
                rightMin = Math.Abs((dynamic)root.Item - (dynamic)current.Item);
            }

            int rootMin = Math.Min(leftMin, rightMin);

            return Math.Min(rootMin, Math.Min(MinGap(root.Left), MinGap(root.Right)));
        }
